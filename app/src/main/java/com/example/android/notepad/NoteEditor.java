...
public class NoteEditor extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 创建一个 Intent，用于返回给调用者的结果。
        final Intent intent = getIntent();

        // 获取触发此 Activity 的 Intent 的动作。
        final String action = intent.getAction();

        if (Intent.ACTION_EDIT.equals(action)) {
            // 如果是编辑操作，将状态设置为编辑模式，并获取需要编辑的数据的 URI。
            mState = STATE_EDIT;
            mUri = intent.getData();
        } else if (Intent.ACTION_INSERT.equals(action) || Intent.ACTION_PASTE.equals(action)) {
            // 如果是插入或粘贴操作，将状态设置为插入模式，并尝试插入一个空记录。
            mState = STATE_INSERT;
            mUri = getContentResolver().insert(intent.getData(), null);

            // 如果插入失败，记录日志并结束 Activity。
            if (mUri == null) {
                Log.e(TAG, "无法插入新笔记：" + getIntent().getData());
                finish();
                return;
            }
